 ...
 * @param {Date} dt  - Date must be a date
 * @param {String} format -If you want to change the format by digits only, write all the letters (formated pattarn) in small letters. Like: (dd.mm.yyyy) -> 25.01.2025. And if you want to write with the month in full name, write only the month letters (formated pattarn) in capital letters. Like: (dd MM, yyyy)-> 25 January, 2025.
 */
export const dateFormat = (date, format) => {

    const check_MM = format.includes('MM');
    const check_mm = format.includes('mm');
    const check_dd = format.includes('dd');
    const check_yyyy = format.includes('yyyy');
    const mmCheck = check_MM || check_mm ? true : false;
    const checkLen = format.length >= 10 && format.length <= 11 ? true : false;

    const conditions = Date.parse(date) && mmCheck && check_dd && check_yyyy && checkLen;

    if (!conditions) {
        return "Your date format is incorrect.";
    }
    ...
    const matchCase = format.match('MM');

    if (matchCase) {
        const replaceInFormat1 = format.replace('yyyy', fullYear);
        const replaceInFormat2 = replaceInFormat1.replace('MM', monthArra[monthIndex + 1]);
        const replaceInFormat3 = replaceInFormat2.replace('dd', days[day]);
        return replaceInFormat3;
    } else {
        const replaceDigit1 = format.replace('yyyy', fullYear);
        const replaceDigit2 = replaceDigit1.replace('mm', days[monthIndex + 1]);
        const replaceDigit3 = replaceDigit2.replace('dd', days[day]);
        return replaceDigit3;
    }
...
export const createRandomArray = (data = [], limit = 10, isRandom = false) => {
    const array = [...data];
    ...
    const fixedLimit = limit > 10 ? 10 : limit;

    if (limit < 0) {
        return array.slice(fixedLimit);
    } else {
        return array.slice(0, fixedLimit);
    }
}
